...
Here is an exmaple ERD for an app that has been setup to perform excel merge on a table called *Reciepts* and a table called *Invoices*. The .xlsx templates are stored in the table called *Templates* and the resultant merged documents are stored in *Merge Docs*. 
...
## Configuration
First copy the [config.template.js file](https://github.com/TrackVia-Sauces/ExcelMerge/blob/master/config.template.js) as `config.js` and then change the following in `config.js` to match the details of your account:
```
/************************* Account Config *********************************/
...
## .xlsx Templates
The [template file included](https://github.com/TrackVia-Sauces/ExcelMerge/blob/master/template_example.xlsx) demonstates how to add rows to an Excel file.
